...
- **JSON contract**: every AJAX endpoint returns `ApiResponse<T>` (`Models/Dto/ApiResponse.cs`): `{ success, message, data, errors }`. HTTP status stays 200; the `success` flag carries the outcome. Success = `Json(ApiResponse.Ok(...))`; failure = `Json(ApiResponse.Fail<object>(...))`. Do not return raw anonymous JSON from controllers.
- **Binding/validation**: POST bodies bind DTO save models from `Models/Dto/` (`UserSaveDto`, `RoleSaveDto`, `LoginRequestDto`) via `[FromBody]`; server-side DataAnnotations are the source of truth, controllers null-guard and return the first `ModelState` error; client JS mirrors validation for UX only.
...
- **Grids**: `GET {Users,Roles}/Grid` return named view models `UserGridRowViewModel`/`RoleGridRowViewModel` (from `Models/ViewModel/`); the camelCase JSON keys must match the DataTables `columns` `data:` names. Client-side DataTables processing (no server-side paging).
