...
- **Tonal Layering:** Depth is primarily communicated through color. The base background is `#f7f9fb`, while interactive components or sidebars sit on surface containers.
...
- **Corner Radius:** 4px (Soft) for buttons and input fields. 0.5rem (8px) for cards and modals.
...
- **Buttons:** 
  ...
  - *Edit:* Teal-tinted ghost — `accent-strong` text, `accent` border, `accent-soft` fill, `accent-container` on hover.
  - *Delete:* Danger-tinted ghost — `on-danger-container` text, `#fca5a5` border, `danger-container` on hover.
...
## Layout Shell Polish
The authenticated app shell (sidebar + topbar + page canvas) matches the Login page's craft language while keeping the dark-navy sidebar for institutional authority.

- **Sidebar:** Layered tonal gradient (top `#22304a` → base `#182033` → bottom `#131a29`) with teal/navy radial auroras and a subtle 1px right hairline. The brand tile uses the Login's `#1b365d → #002046` gradient at 2.5rem with a teal icon glow and a 7s sonar ring pulse (`::after`, capped at 1.4× for collapsed rail). Nav hover uses a teal-300 wash (`rgba(94,234,212,0.08)`); active state has a teal gradient + 3px left rail (`#5eead4`). Caret (`fa-chevron-right`, rotates 90° on expand) + child borders go teal on expansion/hover. Flyout gets gradient bg + 2px teal top stripe. Sidebar toggle icon is `fa-chevron-right` → rotates to down via `transform: rotate(90deg)` when expanded.
- **Topbar:** Frosted glass (`rgba(255,255,255,0.92)` + `backdrop-filter: blur(8px)`, `@supports` fallback to solid white). Title uses weight 650 + `-0.02em` letter-spacing. User profile section: gradient avatar circle (`primary-container → accent`), username from `User.Identity?.Name`, role from `ClaimTypes.Role` — hidden on mobile (`< 768px`). Logout link with danger-red hover. Icon buttons use 8px radius + teal-50 hover wash + 1px lift.
- **Page Canvas:** Login's 3-stop gradient (`#f0fdfa → #f7f9fb → #eff6ff`) with teal/navy radial auroras + a `.rp-bg-pattern` dot overlay + a breathing ambient glow (reuses `rp-login-breathe` keyframes). Content sits at `z-index: 1`.
- **Cards:** `border-radius: 0.5rem` + soft elevation (`0 1px 2px rgba(15,23,42,0.03), 0 4px 16px -8px rgba(15,23,42,0.06)`).
- **Typography:** `.rp-display-lg` uses `-0.02em` letter-spacing; `.rp-headline-md` uses `-0.01em`.
- **Motion:** Sidebar fade entrance (0.4s, 0.1s delay), topbar rise (0.4s, 0.15s delay), page content fade+rise (0.5s, 0.25s delay) — one-time per load, staggered. Guarded by `prefers-reduced-motion` (snap, no animation).
